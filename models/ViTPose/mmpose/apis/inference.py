...
def _inference_single_pose_model(model,
    ...
    if dataset_info is not None:
        ...
        elif dataset in 'ElephantDataset':
            flip_pairs = [[4, 3], [6, 5], [8, 7], [10, 9], [12, 11], [14, 13],
    ...
    #print('batch meta ', batch_data['img_metas'])
    # forward the model
    with torch.no_grad():
        result = model(
            img=batch_data['img'],
...
def _inference_single_pose_model_modified(model,
                                 img_or_path, 
                                 bboxes,
                                 cat_id,
                                 dataset='TopDownCocoDataset',
    ...
    batch_data = collate(batch_data, samples_per_gpu=len(batch_data))
    batch_data = scatter(batch_data, [device])[0]
    #print('bath ch info',batch_data['img_metas'])
...
def inference_top_down_pose_model_modified(model,
                                  img_or_path,
                                  person_results=None,
                                  bbox_thr=None,
                                  format='xywh',
    ...
    """
    # get dataset info
    #print(DatasetInfo(model.cfg.dataset_info_eleph))
    if cat_id ==0:
        dataset_info = DatasetInfo(model.cfg.dataset_info_eleph)
        dataset = 'ElephantDataset'
    elif cat_id ==1:
        dataset_info = DatasetInfo(model.cfg.dataset_info1)
    ...
    with OutputHook(model, outputs=outputs, as_tensor=False) as h:
        # poses is results['pred'] # N x 17x 3
        poses, heatmap = _inference_single_pose_model_modified(
            model,
            img_or_path,
            bboxes_xywh,
            cat_id = cat_id,
...
def vis_pose_result_modified(model,
                    img,
                    result,
                    cat_id = 0,
                    radius=4,
