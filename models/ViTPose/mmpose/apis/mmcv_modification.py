...
class EpochBasedRunnerModified(BaseRunner):
    ...
    def train(self, data_loaders,  **kwargs):
        ...
        iter_counter = sum([len(i) for i in self.data_loaders ])
        
        #self._max_iters = self._max_epochs * (len(self.data_loaders[0])+len(self.data_loaders[1]))
        self._max_iters = self._max_epochs *iter_counter 
                ...
                for _ in range(epochs+1):
                    ...
                    if mode == 'train' and len(data_loaders)>2:
                        if self.epoch <150:
                            ...
                            epoch_runner([data_loaders[i],data_loaders[1]], **kwargs)
                        	
                        
                        	
                        elif self.epoch >=150 and self.epoch <250:
                            epoch_runner([ data_loaders[i],data_loaders[1], data_loaders[2]],  **kwargs) 
                        else:
                        	epoch_runner([ data_loaders[i],data_loaders[i+1], data_loaders[i+2]],  **kwargs) 
