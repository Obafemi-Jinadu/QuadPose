...
class JointsMSELoss_(nn.Module):
        ...
        if output.shape[1]==33:
            ...
            for idx in range(num_joints):
                ...
                if self.use_target_weight:
                    loss += alpha*self.criterion(heatmap_pred * target_weight[:, idx],
                                        heatmap_gt * target_weight[:, idx])
                else:
                    loss += alpha*self.criterion(heatmap_pred, heatmap_gt)


        elif output.shape[1] ==20:
            #output = output[1] 
            ...
            for idx in range(num_joints):
                ...
                heatmap_gt = heatmaps_gt[idx].squeeze(1)
                if self.use_target_weight:
                    loss += beta*self.criterion(heatmap_pred * target_weight[:, idx],
                                        heatmap_gt * target_weight[:, idx])
                else:
                    loss += beta*self.criterion(heatmap_pred, heatmap_gt)

        return loss / num_joints * self.loss_weight


@LOSSES.register_module()
class CombinedTargetMSELoss_(nn.Module):
...
@LOSSES.register_module()
class JointsOHKMMSELoss_(nn.Module):
