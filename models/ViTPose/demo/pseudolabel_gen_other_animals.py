...
def GenPseudo(difficulty='easy'):
    ...
    root = {"info": info, "licenses": licenses, "images": pseudo_image_list, "annotations": pseudo_annotations,
                    "categories": categories,"pseudo_details":pseudo_flag}
    print("number of images", len(pseudo_image_list))
    print("number of annotations", len(pseudo_annotations))

    with open("pseudos_other_keypoints_raw_"+difficulty + ".json", 'w') as fp:
        json.dump(root, fp)
    fp.close()

    ann_path = "/media/obafemi/New Volume/ViTPose/data/annotations"

    if not os.path.exists(ann_path):
        os.makedirs(ann_path)
    shutil.move("pseudos_other_keypoints_raw_"+difficulty+".json", ann_path + "/pseudos_other_keypoints_raw__"+difficulty+".json")
