...
keypoint_name = {
    ...
    6: "tl_tsk", 7: "r_eye",
    8: "l_eye", 9: "rb_ear", 
    10: "lb_ear", 11: "rbt_ear", 
    12: "lbt_ear", 13: "rst_ear", 
    14: "lst_ear", 15: "tr_ear", 
    16: "tl_ear", 17: "trt_ear",
    18: "tlt_ear", 19: "hoof", 
    20: "tail", 21: "rf_elb", 
    22: "lf_elb", 23: "rb_elb",
    24: "lb_elb", 25: "rf_knee", 
    26: "lf_knee", 27: "rb_knee", 
    28: "lb_knee", 29: "rf_foot", 
    30: "lf_foot", 31: "rb_foot", 
    32: "lb_foot"
}
...
class plt_config:
    def __init__(self, dataset_name):
        ...
        # edge , color
        self.EDGES = [
            ([0,1], [255, 0, 0]),  # l_ankle -> l_knee
            ([1,2], [255, 0, 0]),  # l_ankle -> l_knee
            ([2,7], [255, 0, 0]),  # l_ankle -> l_knee
            ([2,8], [255, 0, 0]),  # l_ankle -> l_knee
            ([7,8], [155, 85, 0]),  # l_knee -> l_hip
            ([7,15], [155, 85, 0]),  # l_knee -> l_hip
            ([8,16], [155, 85, 0]),  # l_knee -> l_hip
            ([2,9], [155, 85, 0]),  # l_knee -> l_hip
            ([2,10], [155, 85, 0]),  # l_hip -> l_shoulder
            ([9,19], [0, 0, 255]),  # r_hip -> r_knee
            ([10,19], [0, 0, 255]),  # r_hip -> r_knee
            ([9,21], [0, 0, 255]),  # r_hip -> r_knee
            ([10,21], [0, 0, 255]),  # r_hip -> r_knee
            ([9,22], [17, 25, 10]),  # r_knee -> r_ankle
            ([10,22], [17, 25, 10]),  # r_knee -> r_ankle
            ([21,25], [17, 25, 10]),  # r_knee -> r_ankle
            ([22,26], [17, 25, 10]),  # r_knee -> r_ankle
            ([25,29], [0, 0, 255]),  # r_hip  -> r_shoulder
            ([26,30], [0, 255, 0]),  # l_ear -> l_eye
            ([19,20], [0, 255, 5]),  # l_eye -> r_eye
            ([20,23], [0, 255, 170]),  # l_eye -> nose
            ([20,24], [0, 255, 25]),  # nose -> r_eye
            ([23,27], [0, 17, 255]),  # r_eye -> r_ear
            ([24,28], [0, 220, 0]),  # l_wrist -> l_elbow
            ([27,31], [0, 220, 0]),  # l_elbow -> l_shoulder
            ([28,32], [125, 125, 155]),  # l_shoulder -> r_shoulder
...
def inspect_atten_map_by_locations(
    ...
    fig, axs = plt.subplots(attn_layers_num, len(attn_map_pos) + 1, figsize=(30, 8),)
    fig.subplots_adjust(
    ...
    for id, attn_map in enumerate(enc_atten_maps_hwhw):
        for p_id, p in enumerate(attn_map_pos):
            ...
            if id == attn_layers_num - 1:
                if p_id < len(keypoint_name):
                    axs[id][p_id + 1].set_xlabel(
                        "{}".format(keypoint_name[p_id]), fontsize=12,
                    )
