...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.intermediate_layer_el = nn.Conv2d(
            in_channels=pre_stage_channels[0],
            out_channels=38,
            kernel_size=extra.FINAL_CONV_KERNEL,
            stride=1,
        ...
        self.bn_inter_elep = nn.BatchNorm2d(38)
        self.relu_int = nn.ReLU(inplace=True)
        
        self.final_layer_otherAnimals = nn.Conv2d(
            in_channels=30,
            out_channels=20,
            kernel_size=extra.FINAL_CONV_KERNEL,
            stride=1,
        ...
        self.intermediate_layer_others = nn.Conv2d(
            in_channels=pre_stage_channels[0],
            out_channels=30,
            kernel_size=extra.FINAL_CONV_KERNEL,
            stride=1,
        ...
        self.bn_inter_other = nn.BatchNorm2d(30)
        #self.relu_int = nn.ReLU(inplace=True)
    ...
    def forward(self, x, cls):
        ...
        y_elephant = y_list
        y_list = y_list[0][torch.where(cls!=0)[0]]
        y_elephant = y_elephant[0][torch.where(cls==0)[0]]
        
        
        x_elephant = self.relu_int(self.bn_inter_elep(self.intermediate_layer_el(y_elephant)))
        x = self.relu_int(self.bn_inter_other(self.intermediate_layer_others(y_list)))
        x = self.final_layer_otherAnimals(x)
        x_elephant = self.final_layer(x_elephant)
