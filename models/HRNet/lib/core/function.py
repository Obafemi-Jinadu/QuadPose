...
# Modified by Victor Oludare and Obafemi Jinadu
# ------------------------------------------------------------------------------
...
def _diag_fisher(model: nn.Module, train_loader_list, criterion):
    ...
    ####################
    for train_loader in [train_loader_list[0], train_loader_list[1]]:
            ...
            for i, (input, target, target_weight, meta) in enumerate(train_loader):
                ...
                for n, p in model.named_parameters():
                    if p.grad is not None:
                        precision_matrices[n].data += p.grad.data ** 2 / len(train_loader_list[1].dataset)

    precision_matrices = {n: p for n, p in precision_matrices.items()}
    return precision_matrices, _means

def penalty(_precision_matrices, _means ,model: nn.Module):
    loss = 0
    for n, p in model.named_parameters():
        _loss = _precision_matrices[n] * (p - _means[n]) ** 2
        loss += _loss.sum()
    return loss
...
def train_with_pseudo(config, train_loader_list, model, criterion, optimizer, epoch, output_dir, tb_log_dir, writer_dict,pseudos ):
    ...
    for train_loader_ in train_loader_list:
        
       
        train_loader, alpha = train_loader_
