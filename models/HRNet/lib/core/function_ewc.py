...
class EWC(object):
    def __init__(self, model: nn.Module,  train_loader_list, criterion):

        self.model = model
        self.dataset = train_loader_list
    ...
    def _diag_fisher(self):
        ...
        ####################
        for train_loader in [self.dataset[0],self.dataset[1]]:
            if train_loader != 'pseudo_train_loader':
        
                for i, (input, target, target_weight, meta) in enumerate(train_loader):
                    self.model.zero_grad()
                    #output = model(input, meta['cat_id']).view(1, -1)
                    outputs1, outputs2 = self.model(input, meta['cat_id'])
                    ...
                    if isinstance(outputs, list):
                        loss = self.criterion(outputs[0], target, target_weight)
                    ...
                    for n, p in self.model.named_parameters():
                    	 if p.grad is not None:
                        	precision_matrices[n].data += p.grad.data ** 2 / len(self.dataset[1].dataset)
...
def train_with_pseudo(config, train_loader_list, model, criterion, optimizer, epoch, output_dir, tb_log_dir, writer_dict,pseudos ):
    ...
    if epoch>=151:
        ...
        'state_dict'
        model_.load_state_dict(torch.load('/media/obafemi/New Volume/Animal-Elephant-Pose-Estimation-paper_elephant/progessive_psa_with_all_june24_24/coco/pose_hrnet/w48_384x288_adam_lr1e-3/taskA.pth')['state_dict'])
        ewc = EWC(model_,  train_loader_list, criterion)
    ...
    for train_loader in train_loader_list:
        if train_loader != 'pseudo_train_loader':
            ...
            for i, (input, target, target_weight, meta) in enumerate(train_loader):
                ...
                if epoch>=151:
                    lambda_ = 10000
                    loss += lambda_* ewc.penalty(model)
                    #for name,p in model.named_parameters():
