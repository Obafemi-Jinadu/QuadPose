...
def get_ensemble_predictions(config, hm, center, scale):
    coords, maxvals = get_max_preds(hm)
    preds_ensemble = np.zeros(coords.shape)
    method = ["DAEC", "DARK"]
    preds_daec, maxvals_ensemble = get_final_preds(config, hm, center, scale, mode=method[0])
    preds_dark, _ = get_final_preds(config, hm, center, scale, mode=method[1])

    for j in range(len(preds_daec)):
        for k in range(len(maxvals_ensemble[1])):
            preds_ensemble[j][k][0] = (preds_daec[j][k][0] + preds_dark[j][k][0])/2
            preds_ensemble[j][k][1] = (preds_daec[j][k][1] + preds_dark[j][k][1])/2

    return preds_ensemble, maxvals_ensemble
...
def get_final_preds(config, hm, center, scale, mode="DAEC"):
    ...
    """
    batch_heatmaps = hm.copy()
    modes = ["STANDARD", "SHIFTING", "DARK", "DAEC", "ENSEMBLE"]
    ...
    # post-processing
    if mode in ["STANDARD", "SHIFTING", "DARK", "DAEC", "ENSEMBLE"]:
        if mode == "STANDARD":
            for n in range(coords.shape[0]):
                for p in range(coords.shape[1]):
                    heatmap = batch_heatmaps[n][p]
        ...
        if mode == "ENSEMBLE":
            return get_ensemble_predictions(config, hm, center, scale)
