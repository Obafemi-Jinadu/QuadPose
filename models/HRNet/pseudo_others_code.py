...
file_name =  '/media/obafemi/New Volume/Animal-Elephant-Pose-Estimation-paper_elephant/experiments/coco/hrnet/w32_384x288_adam_lr1e-3.yaml' # ch
...
checkpoint = torch.load('/media/obafemi/New Volume/Animal-Elephant-Pose-Estimation-paper_elephant/hpc_weights/32_final_state.pth')
...
for i, im_name in enumerate(imgs_in):
    ...
    bbox_coords, joints = pred_single(img, bboxes, animal_type='others', model=model, transform=transform_pipeline)
...
for image_list in [(images_list_pets, annotations_pets)]:
    ...
    with open("pseudo_ap-60k_keypoints32_train2017" + ".json", 'w') as fp:
...
if move == True:
    shutil.move("pseudo_ap-60k_keypoints32_train2017.json", ann_path + "/pseudo_ap-60k_keypoints32_train2017.json")
