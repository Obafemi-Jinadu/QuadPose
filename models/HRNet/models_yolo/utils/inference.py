...
def get_ensemble_predictions(config, hm,):
    coords, maxvals = get_max_preds(hm)
    preds_ensemble = np.zeros(coords.shape)
    method = ["DAEC", "DARK"]
    preds_daec, maxvals_ensemble = get_final_preds(config, hm, mode=method[0])
    preds_dark, _ = get_final_preds(config, hm, mode=method[1])
...
def get_final_preds(config, hm, mode="DAEC"):
    """
    ...
    if mode in ["STANDARD", "SHIFTING", "DARK", "DAEC", "ENSEMBLE"]:
        ...
        if mode == "ENSEMBLE":
            return get_ensemble_predictions(config, hm)
