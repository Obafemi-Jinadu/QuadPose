...
file_name =  './experiments/coco/transpose_h/TP_H_w48_256x192_stage3_1_4_d96_h192_relu_enc6_mh1.yaml' # ch
f = open(file_name, 'r')
...
checkpoint = torch.load('') # provide model weights location
...
def pred_single(image,bbox, resolution=(256, 192), animal_type='elephant', decode=False, transform = transform_pipeline, model=model):
...
for image_list in [(images_list_pets, annotations_pets)]:
    ...
    with open("pseudo_48_ap-60k_keypoints_train2017" + ".json", 'w') as fp:
     
        json.dump(root, fp)
    fp.close()

ann_path = "" #specifiy annotation path
move = True
if not os.path.exists(ann_path):
    os.makedirs(ann_path)
if move == True:
    shutil.move("", ann_path + "/")
