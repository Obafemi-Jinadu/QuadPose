...
file_name =  './experiments/coco/transpose_h/TP_H_w48_256x192_stage3_1_4_d96_h192_relu_enc6_mh1.yaml' 
f = open(file_name, 'r')
...
for i, im_name in enumerate(imgs_in):
    ...
    for pts_idx, coord in enumerate(coords):
        if coord[-1]>=0.9:# and coord[-1]<0.9:
...
for image_list in [(images_list_pets, annotations_pets)]:
    ...
    with open("pseudo_48_elephant1_keypoints_train2017" + ".json", 'w') as fp:
